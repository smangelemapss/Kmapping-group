...
# -------------------------------------------------
k = 3  # Change this to 3 for part 2
...
print("\n--- ENERGY STATISTICS ---")
print(f"Minimum Energy: {df['Energy'].min()} (Node: {df.loc[df['Energy'].idxmin(), 'Node']})")
print(f"Maximum Energy: {df['Energy'].max()} (Node: {df.loc[df['Energy'].idxmax(), 'Node']})")
print(f"Mean Energy: {df['Energy'].mean():.2f}")
print(f"Energy Std Dev: {df['Energy'].std():.2f}")

print("\n--- ENERGY PER CLUSTER ---")
cluster_stats = df.groupby('Cluster')['Energy'].agg(['count', 'min', 'mean', 'max', 'std'])
print(cluster_stats.round(2))

print("\n--- CRITICAL NODES (Lowest Energy per Cluster) ---")
for cluster in sorted(df['Cluster'].unique()):
    cluster_df = df[df['Cluster'] == cluster]
    min_node = cluster_df.loc[cluster_df['Energy'].idxmin()]
    print(f"Cluster {cluster}: Node {min_node['Node']} | Energy: {min_node['Energy']} | Position: ({min_node['X']:.2f}, {min_node['Y']:.2f})")
...
print("\n--- LOWEST ENERGY NODES GLOBALLY ---")
lowest_5 = df.nsmallest(5, 'Energy')[['Node', 'Energy', 'X', 'Y', 'Cluster']]
print(lowest_5)

print("\n--- HIGHEST ENERGY NODES GLOBALLY ---")
highest_5 = df.nlargest(5, 'Energy')[['Node', 'Energy', 'X', 'Y', 'Cluster']]
print(highest_5)
...
# -------------------------------------------------
# 8. Smooth 3D surface plot (improved)
# -------------------------------------------------
